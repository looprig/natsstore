...
Path discovery is frozen during `Open`. Every `StoragePaths` call returns a defensive clone, so callers cannot mutate later results. The outer `Store` and each concrete persistence primitive (`ledgerStore`, `leaserStore`, `kvStore`, and `blobStore`) receive the same immutable private reporter. This matters because callers often hand out `Store.Backend()` and then see only the primitive interface fields on `storage.Composite`; each provider remains independently discoverable with a `storage.PathReporter` type assertion.
